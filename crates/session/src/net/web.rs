...
/// Where a page can learn WhatsApp Web's current version.
///
/// Not from WhatsApp: `web.whatsapp.com/sw.js` sends no
/// `Access-Control-Allow-Origin`, so a browser refuses to let this origin
/// read it. This is a third party that tracks the same number and *does*
/// serve it cross-origin, over a CDN, which is the only shape that works from
/// a static page with no server behind it.
///
/// What is trusted to it is a version triple and nothing else: three integers
/// parsed out of one field, used to fill in a number the desktop would have
/// read from WhatsApp directly. A wrong one costs a refused connection, which
/// the fallback below then does not repair — that is the honest cost of not
/// having a server, and it is why the answer is validated rather than adopted.
const VERSION_FEED: &str =
    "https://cdn.jsdelivr.net/gh/wppconnect-team/wa-version@main/versions.json";

/// How long the version lookup may take before the built-in one is used.
///
/// Short, because this is in front of everything: the socket, the pairing QR,
/// the whole session wait on it. A CDN that is slow today is not a reason for
/// a page that never loads.
const VERSION_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(5);

/// The version to announce, fetched if it can be and compiled in if not.
///
/// Never fails. The library ships a constant that was current when the
/// dependency was built, and that is a far better answer than no session at
/// all — so every way this can go wrong ends there.
pub(crate) async fn app_version() -> (u32, u32, u32) {
    let built_in = whatsapp_rust::version::WA_WEB_VERSION;
    match crate::exec::with_timeout(fetch_version(), VERSION_TIMEOUT).await {
        Some(Ok(version)) => {
            log::info!(
                "using WhatsApp Web version {}.{}.{} from {VERSION_FEED}",
                version.0,
                version.1,
                version.2
            );
        ...
        Some(Err(e)) => {
            log::warn!("could not read the current version ({e}); using the built-in one");
            built_in
        }
        None => {
            log::warn!("the version lookup did not answer in time; using the built-in one");
            built_in
        }
    }
}

/// Read `currentVersion` out of the feed.
async fn fetch_version() -> Result<(u32, u32, u32)> {
    use wasm_bindgen::JsCast as _;
    use wasm_bindgen_futures::JsFuture;

    let window = web_sys::window().ok_or_else(|| anyhow!("no window to fetch from"))?;
    let response = JsFuture::from(window.fetch_with_str(VERSION_FEED))
        .await
        .map_err(|e| anyhow!("{e:?}"))?
        .dyn_into::<web_sys::Response>()
        .map_err(|_| anyhow!("the version feed answered with something that is not a response"))?;
    if !response.ok() {
        return Err(anyhow!("the version feed answered {}", response.status()));
    }
    let text = JsFuture::from(response.text().map_err(|e| anyhow!("{e:?}"))?)
        .await
        .map_err(|e| anyhow!("{e:?}"))?
        .as_string()
        .ok_or_else(|| anyhow!("the version feed's body was not text"))?;

    let feed: serde_json::Value =
        serde_json::from_str(&text).map_err(|e| anyhow!("the version feed is not JSON: {e}"))?;
    let named = feed
        .get("currentVersion")
        .and_then(serde_json::Value::as_str)
        .ok_or_else(|| anyhow!("the version feed named no currentVersion"))?;
    super::parse_version(named).ok_or_else(|| anyhow!("could not read a version out of {named:?}"))
}
