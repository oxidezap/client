-- Roll back the stable-id rewrite. Rowids are reassigned by the rebuild
-- (SQLite numbers fresh rows `max(rowid) + 1`), so arrival cursors and the
-- FTS mapping do not survive a downgrade the way they survive the upgrade;
-- the FTS objects are dropped here and rebuilt by `ensure_fts` at next open.
DROP TRIGGER IF EXISTS messages_fts_ai;
DROP TRIGGER IF EXISTS messages_fts_ad;
DROP TRIGGER IF EXISTS messages_fts_au;
DROP TABLE IF EXISTS messages_fts;

CREATE TABLE messages_old (
    device_id INTEGER NOT NULL,
    chat_jid TEXT NOT NULL,
    msg_id TEXT NOT NULL,
    sender_jid TEXT NOT NULL,
    from_me BOOLEAN NOT NULL DEFAULT FALSE,
    timestamp_ms BIGINT NOT NULL,
    kind TEXT NOT NULL,
    text_content TEXT,
    proto BLOB,
    status INTEGER NOT NULL DEFAULT 1,
    starred BOOLEAN NOT NULL DEFAULT FALSE,
    edited_at_ms BIGINT,
    revoked BOOLEAN NOT NULL DEFAULT FALSE,
    PRIMARY KEY (device_id, chat_jid, msg_id, sender_jid)
);

-- Compressed protos cannot go back: the old shape has no codec slot, and
-- SQLite's RAISE() is trigger-only so the guard cannot live in this SELECT.
-- Downgrading a store with codec != 0 rows copies the compressed bytes
-- through; the old reader fails their decode with a warning and renders the
-- denormalized columns (text/kind) without the rich proto. Degraded, not
-- corrupt — but only downgrade a store you can afford to re-sync.
INSERT INTO messages_old
    (device_id, chat_jid, msg_id, sender_jid, from_me, timestamp_ms, kind,
     text_content, proto, status, starred, edited_at_ms, revoked)
SELECT device_id, chat_jid, msg_id, sender_jid, from_me, timestamp_ms, kind,
       text_content, proto, status, starred, edited_at_ms, revoked
  FROM messages;

DROP TABLE messages;
ALTER TABLE messages_old RENAME TO messages;

CREATE INDEX idx_messages_chat_time
    ON messages (device_id, chat_jid, timestamp_ms);
CREATE INDEX idx_messages_by_id ON messages (device_id, msg_id);
