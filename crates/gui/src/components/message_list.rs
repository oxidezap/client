                    ...
                    move |app, visible_range, _scroll_handle, cx| {
                        ...
                        visible_range
                            ...
                            .map(|ix| {
                                ...
                                // Decoded once and reused: stickers additionally
                                // need the stable Arc for animation state.
                                let sticker_image = msg.media.as_ref().and_then(|m| {
                                    if matches!(m.media_type, MediaType::Sticker | MediaType::Image)
                                        && !m.data.is_empty()
                                    {
                                        Some(app.get_decoded_image(
                                            message_id,
                                            &m.data,
                                            &m.mime_type,
                                        ))
